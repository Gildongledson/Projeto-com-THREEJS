...
function init() {
  ...
  const groudMath = new THREE.MeshBasicMaterial({
    color: 0xffffff,
    side: THREE.DoubleSide,
    //wireframe: true
  });

  groundMesh = new THREE.Mesh(groundGeo, groudMath);
  ...
  boxBody = new CANNON.Body({
    mass: 1,
    //A função CANNON.Box() recebe um vetor com as dimensões do cubo divididas por 2.
    //Um cubo de (15, 15, 15) deve receber um vetor de 7.5, 7.5, 7.5.
    shape: new CANNON.Box(new CANNON.Vec3(7.5, 7.5, 7.5)),
    //Posição do cubo é contada a partir do centro, com valor y um pouco acima do chão.
    position: new CANNON.Vec3(13, -8, 25)
  });
  ...
  //define a gravidade do mundo como -9.82 no eixo y
...
function render() {
  ...
  cubo.position.copy(boxBody.position);
  cubo.quaternion.copy(boxBody.quaternion);
